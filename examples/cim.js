...
util.codegen = __webpack_require__(43);
util.fetch   = __webpack_require__(44);
util.path    = __webpack_require__(45);
...
util.LongBits = __webpack_require__(39);
...
rpc.Service = __webpack_require__(42);
...
function getInstance(options) {
  mySocket = {
    ...
    roomId: '', // 房间ID
    isAnonymous: options.isAnonymous, // 游客模式
      ...
      switch (mySocket.roomState) {
        ...
        case _config2.default.RoomState.ROOM_LIVE:
          ...
          if ((0, _util.GetCurLocalTime)() - mySocket.roomUpdateTime > 30) {
            ...
            console.log('直播间心跳超时:' + mySocket.roomId);
            break;
          }
          if ((0, _util.GetCurLocalTime)() - mySocket.roomHeartBeatTime > 10) {
            if (mySocket.roomId) {
              ...
              mySocket.SendPbMsg(true, 'live_room', mySocket.account, heartBeat, '', '', {
  ...
  mySocket.onSocketMessage = function (res) {
    ...
    if (typeof getMsg === 'string') {} else {
      ...
      reader.onload = function (e) {
        ...
        if (!pbMsgHead.pbName) {
          console.error('get err msg:', pbMsgHead);
          if (pbMsgHead) {
            return;
          }
        }
        ...
        if (!pbMsgBuilder) {
          console.error('net->收到未知消息：' + pbMsgHead.pbName);
          return;
        }
        // 消息解码
        var pbMsgRsp = pbMsgBuilder.decode(pbMsgHead.pbData);
        if (pbMsgHead.pbName !== 'pb_pub.HeartBeat') {
          // console.log(pbMsgHead, pbMsgRsp, '每次收到的消息2', pbMsgHead.errCode)
        }
        if (pbMsgRsp.result) {
          console.error('失败原因：' + pbMsgRsp.result);
          switch (pbMsgHead.pbName) {
            case 'pb_msg_gate.LoginRsp':
              // 登录结果返回 result 成功0
              if (pbMsgRsp.result) {
                // 登录失败
                console.log('net => Login error! result:' + pbMsgRsp.result);
                mySocket.SetNetState(_config2.default.NetState.NET_STATE_INIT);
                mySocket.ifReset = false;
                mySocket.Stop();
                options.onerror({
        ...
        switch (pbMsgHead.pbName // pbMsgRsp.result 错误码 0:代表成功
          ...
          case 'pb_msg_gate.LoginRsp':
            ...
            if (!mySocket.isAnonymous) {
              ...
              console.log(loginRes.msgSn.toString(), '正常用户登录返回的sn');
            } else {
              mySocket.account = pbMsgHead.pbCommData.srcId;
              mySocket.msgSn = loginRes.msgSn;
              console.warn('游客模式下的用户id' + mySocket.account);
            }
          ...
          case 'pb_pub.MsgReceipt':
            ...
            console.log(obj, pbMsgRsp, '消息状态接收');
            if (pbMsgRsp.time) {
          ...
          case 'pb_pub.YXCommMsg':
            ...
            // console.log(getMsg, '收到的消息-----------IM')
            options.onmsgs(_getMsg);
            // let type = getMsg.MessageType
            // let alltype = [1, 5, 6]
            // if (alltype.indexOf(type) !== -1) {
            //   console.log(getMsg, '收到的消息')
            // }
            break;
  ...
  mySocket.reConnectIm = function () {
    ...
    mySocket.reconnectTimer = setInterval(function () {
      (0, _network.netstatus)().then(function () {
        // console.log(netConnect, '测试网络！！！')
        if ((0, _util.GetCurLocalTime)() - recTime > 100) {
          // 退出直播间
          mySocket.roomId = '';
          clearInterval(mySocket.reconnectTimer);
          // console.log('断网时间过长，退出直播间')
  ...
  mySocket.enterLiveRoom = function (params, opts) {
    ...
    // console.log(configs, '聊天室连接房间Id')
    mySocket.SendPbMsg(true, 'live_room', mySocket.account, pbMsg, opts, '', configs);
  };
  /**
   * 断开聊天室
   */
  ...
  mySocket.sendLiveRoomMsg = function (params, opts) {
    ...
    };mySocket.SendPbMsg(true, 'live_room', mySocket.account, pbMsg, opts, params.done, configs);
  };
   ...
   *
   */
  mySocket.SendPbMsg = function (needReadReceipt, service, hashKey, pbMsg, opts, done, configs) {
    ...
    // 处理回调数据
    if (pbMsgHead.pbName !== '.pb_pub.HeartBeat' && done) {
      var msgObj = {};
      msgObj.pbMsg = pbMsg;
      msgObj.state = 0;
      msgObj.pbCommData = pbMsgHead.pbCommData;
      msgObj.pbCommData = (0, _util.parseLong)(msgObj.pbCommData);
      // console.log(pbMsg, '999999999999999999996')
      done('发送成功', msgObj);
    }
    // 最终发送的pb数据
    if (pbMsgHead.pbName !== '.pb_pub.HeartBeat') {}
    // console.log(pbMsgHead.pbCommData.msgSn.toString(), '最终发送的消息sn')
    // console.log(pbMsgHead, pbMsg, '最终发送的消息头，消息体');
...
var protobuf = __webpack_require__(29);
var Long = __webpack_require__(48);
...
var config = {
  ...
  globalData: {},
  Long: Long,
  pbjsonurl: 'https://static.hnyequ.cn/h5/js/pbmsg/client_pb.json?refresh=' + time
  /**
   * err 错误信息
   * root 加载的文件信息
   */
...
protobuf.configure();
protobuf.load(config.pbjsonurl, function (err, root) {
  if (err) {
    console.log('error loading protobuf file:config.js');
    return;
  }
  config.pbMsgRoot = root;
...
protobuf.parse            = __webpack_require__(46);
protobuf.common           = __webpack_require__(47);
...
var protobuf = module.exports = __webpack_require__(32);
...
protobuf.BufferWriter = __webpack_require__(40);
