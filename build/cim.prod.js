...
function getInstance(options) {
  ...
  mySocket.initConnect = function (options) {
    ...
    var localsn = localStorage.getItem('msgsn');
    if (localsn && localsn !== 'null' && localsn !== 'undefined') {
      mySocket.msgSn = _config2.default.Long.fromValue(localsn, false);
    }
  ...
  mySocket.enterLiveRoom = function (params, opts) {
    ...
    if (!mySocket.roomId || mySocket.roomId === 'undefined' || mySocket.roomId === 'null') {
      console.log('enterLiveRoom无roomId参数');
      mySocket.Stop();
      options.onerror({
        message: 'roomId不存在，重新登录',
        code: 101 // java重新获取token
...
var config = {
  ...
  globalData: {},
  Long: Long,
  pbjsonurl: 'https://static.hnyequ.cn/h5/js/pbmsg/client_pb.json'
  /**
   * err 错误信息
   * root 加载的文件信息
   */
